...
public:
    ...
    undirected_edge(I nodo_primo, I nodo_secondo) {
        nodo1 = nodo_primo;
        nodo2 = nodo_secondo;
    if (nodo1 > nodo2) {
...
std::list<I> neighbours(I nodo) {
    return lista_adiacenza[nodo];
}

void add_edge(I nodo_a, I nodo_b) {
        lista_adiacenza[nodo_a].push_back(nodo_b);
        lista_adiacenza[nodo_b].push_back(nodo_a);
        archi_inseriti.push_back(undirected_edge<I> (nodo_a, nodo_b));
}
